...
void swire_add_box_octilinear(const OdbDb& h, rust::Str net, bool fixed, rust::Str layer,
                              int32_t x1, int32_t y1, int32_t x2, int32_t y2,
  ...
  // is carried separately. Passing a bbox here would place a rectangle where a 45 belongs.
  odb::dbSBox::create(w, l, x1, y1, x2, y2, odb::dbWireShapeType(s(shape).c_str()),
                      odb::dbSBox::OCTILINEAR, width);
}
...
std::size_t swire_clear_routed(const OdbDb& h, rust::Str net) {
  ...
  auto wires = n->getSWires();
  for (auto it = wires.begin(); it != wires.end();) {
    if ((*it)->getWireType() == odb::dbWireType::FIXED) { ++it; continue; }
    it = odb::dbSWire::destroy(it);
    ++removed;
  }
  return removed;
}
