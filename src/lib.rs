...
mod ffi {
    unsafe extern "C++" {
        ...
        fn bterm_set_io_type(db: &OdbDb, bterm: &str, io_type: &str) -> Result<()>;
        fn block_set_def_units(db: &OdbDb, units: i32) -> Result<()>;
        fn read_lef(db: &OdbDb, lef_path: &str) -> Result<()>;
        fn block_set_bus_delimiters(db: &OdbDb, left: &str, right: &str) -> Result<()>;
        ...
        fn reverse_guides(db: &OdbDb, net: &str) -> Result<bool>;
        ...
        fn ndr_use_vias(db: &OdbDb, ndr: &str) -> Result<Vec<String>>;
        fn ndr_use_via_rules(db: &OdbDb, ndr: &str) -> Result<Vec<String>>;
        fn ndr_layer_rule_wire_exts(db: &OdbDb, ndr: &str) -> Result<Vec<i32>>;
        ...
        fn net_guides(db: &OdbDb, net: &str) -> Result<Vec<String>>;
        fn net_wire_decode(db: &OdbDb, net: &str) -> Result<Vec<String>>;
        fn layer_lef58_type(db: &OdbDb, layer: &str) -> String;
        fn layer_rule_census(db: &OdbDb, layer: &str) -> Vec<String>;
        ...
        fn block_reset_gcell_grid(db: &OdbDb, x0: i32, nx: i32, sx: i32, y0: i32, ny: i32, sy: i32) -> Result<()>;
        ...
        fn special_wire_boxes(db: &OdbDb) -> Result<Vec<i64>>;
        fn wire_shape_type_name(value: i32) -> Result<String>;
        #[allow(clippy::too_many_arguments)]
        ...
        fn master_mterms(db: &OdbDb, master: &str) -> Result<Vec<String>>;
        fn mterm_pin_boxes(db: &OdbDb, master: &str, term: &str) -> Result<Vec<i32>>;
        fn mterm_pin_boxes_excluding_polygons(
            db: &OdbDb,
            master: &str,
            term: &str,
        ) -> Result<Vec<i32>>;
        fn iterm_pin_polygons(db: &OdbDb, iterm: &str) -> Result<Vec<i32>>;
        fn net_new_swire(db: &OdbDb, net: &str, fixed: bool) -> Result<()>;
        fn iterm_bool_property(db: &OdbDb, iterm: &str, name: &str) -> Result<i32>;
        fn mterm_bool_property(db: &OdbDb, master: &str, term: &str, name: &str) -> Result<i32>;
        fn polygon_bloat(pts: &[i32], margin: i32) -> Result<Vec<i32>>;
        fn rdl_preprocess(source_rects: &[i32], dest_rects: &[i32], min_dist: i32) -> Result<Vec<i32>>;
        ...
        fn bterm_get_id(db: &OdbDb, bterm: &str) -> Result<u32>;
        ...
        fn net_swire_obstacles(db: &OdbDb, net: &str) -> Result<Vec<i64>>;
        fn tech_via_layer(db: &OdbDb, via: &str, which: &str) -> Result<String>;
        fn layer_get_area(db: &OdbDb, layer: &str) -> Result<i64>;
        fn layer_v55_spacing_table(db: &OdbDb, layer: &str) -> Result<Vec<u32>>;
        fn net_first_driver_term(db: &OdbDb, net: &str) -> Result<String>;
        fn net_swire_expanded_boxes(db: &OdbDb, net: &str) -> Result<Vec<i64>>;
        fn num_v54_spacing_rules(db: &OdbDb, layer: &str) -> Result<usize>;
        fn v54_spacing_rule_eol(db: &OdbDb, layer: &str, idx: usize) -> Result<Vec<i32>>;
        ...
        fn net_has_wire(db: &OdbDb, net: &str) -> bool;
        fn techvia_has_string_property(db: &OdbDb, via: &str, name: &str) -> bool;
        fn v54_spacing_rule_spacing(db: &OdbDb, layer: &str, idx: usize) -> u32;
        fn v54_spacing_rule_range(db: &OdbDb, layer: &str, idx: usize) -> Vec<u32>;
        ...
        // A diagonal (octilinear) special wire -- endpoints plus a width, see shim.h.
        #[allow(clippy::too_many_arguments)]
        fn swire_add_box_octilinear(db: &OdbDb, net: &str, fixed: bool, layer: &str,
                                    x1: i32, y1: i32, x2: i32, y2: i32,
                                    width: i32, shape: &str) -> Result<()>;
        ...
        fn mterm_has_gate_area(db: &OdbDb, master: &str, term: &str) -> bool;
...
pub use ffi::{
    ...
    block_set_core_area_from_rows, block_set_die_area, block_set_die_area_polygon, bterm_direction, bterm_net, bterm_x, bterm_y, check_3dblox, clear_guides, reverse_guides,
    ...
    block_cut_rows, block_cut_rows_at_blockages, group_get_type, has_one_site_master, nth_row_name, site_get_class, create_physical_inst, inst_bbox, master_placement_boundary, master_edge_type_names, master_edge_type_params, tech_cell_edge_spacing_types, tech_cell_edge_spacing_params, ndr_create, ndr_layer_rule_set, ndr_layer_rule_layers, ndr_layer_rule_params, ndr_add_use_via, ndr_use_vias, ndr_use_via_rules, ndr_layer_rule_wire_exts, block_access_point_count, iterm_pref_access_points, iterm_access_point_count, bpin_access_points, net_guides, net_wire_decode, layer_lef58_type, layer_rule_census, mpin_add_access_point, iterm_set_access_point, bpin_add_access_point, net_write_wire, block_create_via, block_set_gcell_grid, block_reset_gcell_grid, num_masters, nth_master_name, master_get_type,
    ...
    die_area_polygon, master_obstruction_boxes, master_pin_boxes, master_mterms, mterm_pin_boxes,
    mterm_pin_boxes_excluding_polygons, iterm_pin_polygons, net_new_swire, iterm_bool_property, mterm_bool_property, polygon_bloat, rdl_preprocess, mpin_boxes, inst_halo,
    net_destroy, iterm_get_id, bterm_get_id, inst_get_id, layer_find_v55_spacing, layer_find_tw_spacing, layer_get_area, layer_v55_spacing_table, net_first_driver_term, net_swire_expanded_boxes, v54_spacing_rule_spacing, v54_spacing_rule_range, techvia_has_string_property, net_has_wire, layer_get_spacing_for, layer_min_area, tech_via_boxes, tech_via_layer, bpin_layer_boxes, region_boundaries, net_swire_shapes, net_swire_obstacles, num_v54_spacing_rules, v54_spacing_rule_eol, v54_spacing_rule_adjacent_cuts, v54_spacing_rule_adjacent_spacing, v54_spacing_rule_adjacent_except_same_pgnet, num_array_spacing_rules, array_spacing_rule_cut_class, array_spacing_rule_is_parallel_overlap, array_spacing_rule_is_long_array, array_spacing_rule_array_width, array_spacing_rule_cut_spacing, array_spacing_rule_cuts_spacing, num_cut_spacing_table_rules, cut_spacing_table_max_spacing, cut_spacing_table_spacing, cut_spacing_table_is_center_and_edge, cut_spacing_table_is_center_to_center, num_width_table_rules, width_table_rule_is_wrong_direction, width_table_rule_widths, techvialayerrule_rect, via_create_generated, swire_add_via, swire_add_box, swire_add_box_shaped, swire_add_box_octilinear, swire_clear_routed, bterm_create, bterm_create_pin, bterm_clear_unfixed_bpins, bterm_add_pin_box, bterm_destroy, layer_get_type,
