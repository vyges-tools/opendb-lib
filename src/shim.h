...
bool mterm_has_gate_area(const OdbDb& db, rust::Str master, rust::Str term);
...
// Each terminal of a master as "name\tSIGTYPE" (POWER/GROUND/SIGNAL/…). Empty if not found.
rust::Vec<rust::String> master_mterms(const OdbDb& db, rust::Str master);
...
int64_t layer_get_area(const OdbDb& db, rust::Str layer);
rust::Vec<uint32_t> layer_v55_spacing_table(const OdbDb& db, rust::Str layer);
